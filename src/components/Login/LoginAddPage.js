...
function LoginAddPage() {
    ...
    const handleSubmit = async (event) => {
        ...
        if (!passwordRegex.test(password)) {
            alert("비밀번호는 영문자와 숫자의 조합으로 8자 이상 20자 이하여야 합니다.");
            return;
        }
        ...
        try {
            const response = await fetch('http://localhost:8080/v1/api/sign/register', {
                method: 'POST',
                headers: {
                ...
                <div className="add-account-content">
                    <div className="greeting-container">
                        ...
                        <span className="login-description">
                            브랜디 계정 하나로 브랜디, 하이버, 마미 서비스를 모두 이용할 수 있어요.
                        </span>
                    </div>
