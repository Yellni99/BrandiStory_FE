...
function ProductDetail() {
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      try {
        const response = await axios.get(`http://localhost:8080/v1/api/products/${productId}`);
        const productData = response.data;
        const mainImg = productData.image_list.length > 0 ? productData.image_list[0].image : '';
  ...
  const getNextDayDate = () => {
    ...
    return nextDay.toLocaleDateString('ko-KR', { year: 'numeric', month: 'long', day: 'numeric' });
  };
  ...
  return (
      <div className="product-box">
        <div className="left-section">
          <div className="main-image">
            {detailedImage[0] && (
            <img src={detailedImage[0]} alt="Main" />
            )}
          </div>
      ...
      {/*</div>*/}
        <div className="right-section">
          ...
          <div className="product-price">{productDetails.price.toLocaleString()}원</div>
